# Script to make SLiM job script for Serial Bottleneck model
...
// After burn-in, create island population of 10 individuals for Serial Bottleneck model
${t} {
    sim.addSubpopSplit("p2", 10, p1);
}

// Double island population size each generation until long-term size is reached
${t}: { 
	newSize_p2 = asInteger(round(2^(sim.generation - ${t}) * 10)) ; 
	if (newSize_p2 >= ${n}) { 
		newSize_p2 = ${n} ;
		sim.deregisterScriptBlock(self) ; }
	p2.setSubpopulationSize(newSize_p2) ; 
}
...
// Double island population size each generation until long-term size is reached
$((${t} + 2000)): { 
	newSize_p2 = asInteger(round(2^(sim.generation - $((${t} + 2000))) * 10)) ; 
	if (newSize_p2 >= ${n}) { 
		newSize_p2 = ${n} ;
		sim.deregisterScriptBlock(self) ; }
	p2.setSubpopulationSize(newSize_p2) ; 
